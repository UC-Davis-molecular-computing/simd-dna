...
def convert_tm_to_simd(instructions):
    ...
    with open(filename, 'r') as file:
        ...
        generate_tm_to_simd_data_from_transitions(transition_data, data, register_name, instructions)
...
def generate_tm_to_simd_data_from_transitions(transition_data, tm_data, register_name, instructions):
    Cell.cell_types.clear()
    Strand.strand_types.clear()
    ...
    generate_tm_instructions(transition_data, tm_data['blank'], instructions)
...
def generate_tm_instructions(transition_data, blank_symbol, instructions):
    ...
    for i in range(num_configurations):
        ...
        if 'L' in transition.keys():
            create_left_instruction(i, num_configurations, transition, transition_data, blank_symbol, instructions)
        else:
            create_right_instruction(i, num_configurations, transition, transition_data, blank_symbol, instructions)

    # Restart instruction
    instructions.append(['({},{})_restart'.format(*c) for c in configurations])


def create_left_instruction(index, num_configurations, transition, transition_data, blank_symbol, instructions):
    ...
    if next_index == -1:
        ...
        generate_left_final_instruction_strands(next_index, num_configurations, configurations, instructions)
...
def generate_left_final_instruction_strands(next_index, num_configurations, configurations, instructions):
...
def create_right_instruction(index, num_configurations, transition, transition_data, blank_symbol, instructions):
